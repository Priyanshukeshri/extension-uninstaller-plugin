...
function activate(context) {
    ...
    const showExtensionsCommand = vscode.commands.registerCommand("extension-uninstaller.plugin-uninstaller", () => {
        ...
        panel = vscode.window.createWebviewPanel("extensionList", "Installed Extensions", vscode.ViewColumn.One, {
...
function askForUninstallConfirmation(extensionId) {
    vscode.window
...
function uninstallExtensionById(extensionId) {
    vscode.commands
...
function getWebviewContent(extensions, extensionPath) {
    // Generate the HTML content for the webview
    const extensionsWithIcon = extensions.filter((extension) => extension.iconUrl !== "");
...
function getExtensionIconUri(extension) {
    // Check if the extension has an icon path specified
    const iconPath = extension.packageJSON.icon;
    if (iconPath) {
        const iconUri = vscode.Uri.file(path.join(extension.extensionPath, iconPath));
        const iconData = fs.readFileSync(iconUri.fsPath).toString("base64");
        const iconMimeType = path.extname(iconPath) === ".svg" ? "image/svg+xml" : "image/png";
